...
DeviceLoop:
	for _, blockDevice := range blockDevices {
		...
		for name, filter := range filterMap {
			...
			if lvset.Spec.DeviceInclusionSpec == nil {
				valid, err = filter(blockDevice, localv1alpha1.DeviceInclusionSpec{})
			} else {
				valid, err = filter(blockDevice, *lvset.Spec.DeviceInclusionSpec)
			}
			if err != nil {
