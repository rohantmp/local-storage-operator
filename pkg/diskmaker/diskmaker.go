	...
	for storageClass, deviceArray := range deviceMap {
		for _, deviceNameLocation := range deviceArray {
			...
			if len(existingSymlinks) > 0 { // already claimed, fail silently
				continue
			} else if errors.Is(err, wait.ErrWaitTimeout) { // timed out waiting for lock
				klog.Warningf("not symlinking, timed out trying to get device lock after: %v", internal.DefaultLockTimeout)
				continue
